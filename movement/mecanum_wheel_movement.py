...
class Wheel:
    ...
    def move(self, power):
        ...
        self.en.duty_cycle = getPWMPer(power) if power > 0 else getPWMPer(
            -power)
...
class Encoder:
    ...
    def readSpeed(self):
        ...
        print(self.name + " position: " + str(self.counter) + " @: " + str(
            self.time) + \
              " speed rev/sec: " + str(
            self.speed * 1E9 / perRev))  # for diagnostic
...
def front_left(power, forSecs):
    print(
        "Front left ahead @ " + str(power) + "% for " + str(forSecs) + " secs.")
    fl.move(power)
    time.sleep(forSecs)
    test_speed()
...
def front_right(power, forSecs):
    print("Front right ahead @ " + str(power) + "% for " + str(
        forSecs) + " secs.")
    fr.move(power)
    time.sleep(forSecs)
    test_speed()
...
def rear_left(power, forSecs):
    print(
        "Rear left ahead @ " + str(power) + "% for " + str(forSecs) + " secs.")
    rl.move(power)
    time.sleep(forSecs)
    test_speed()
    stop_car()  # will set speed to 0
...
def rear_right(power, forSecs):
    print(
        "Rear right ahead @ " + str(power) + "% for " + str(forSecs) + " secs.")
    rr.move(power)
    time.sleep(forSecs)
    test_speed()
    stop_car()  # will set speed to 0
...
def test_move():
    ...
    print(
        "Run forward all, full power for 1 sec, then coast all for 3 sec, then stop")
    rl.move(100)
    rr.move(100)
    fr.move(100)
    rr.move(100)
...
def main():
    ...
    actionList = [front_left(100, 2), front_right(100, 2), rear_left(100, 2),
                  rear_right(100, 2)]
    #    while True:
    for x in actionList:
        x;
